...
async function loadData() {
    try {
        ...
        // Usar dados incorporados diretamente (mais confiável)
        console.log('Usando dados incorporados para maior confiabilidade');
        let csvText;
            
            // Dados de fallback incorporados diretamente no código
            csvText = `data;tipo_contribuicao;tipo_autor;autor;id_autor;texto;id_proposta;secao;mudanca_proposta;risco_implementacao;divergencia;recomendacao;comentarios_coalizao;decisao_mma;comentarios_mma
...
08/09/2025 08:42;Individual;Pessoa Física;HENRIQUE;1643611;As chamadas de apoio a pesquisa desenvolvimento e inovação podem conter critério / diretrizes que valorizem aqueles projetos de ICTs e empresas que estejam em parceria com as cooperativas participantes dos elos iniciais das cadeias da bioeconomia.;42628;Ação Estratégica 10: Apoiar coo...;Inserir texto;Médio;;Considerar parcialmente;Sugere-se a consideração desse ponto, mas com restrições d epontuação adicional, visando equilíbrio entre diversos elos;;`;
        ...
        const errorMessage = `Erro: ${error.message}. Tentando usar dados de exemplo...`;
        
        document.querySelectorAll('.highlight').forEach(el => {
            el.textContent = '0';
        });
        
        document.getElementById('most-commented-section').textContent = 'Carregando...';
            ...
            container.innerHTML = `
                ...
                <div class="loading">
                    ...
                    <p style="color: #7f8c8d; font-size: 0.9em; text-align: center; margin-top: 10px;">
                        Verifique se o arquivo CSV está na mesma pasta ou recarregue a página.
                    </p>
                </div>
            `;
        });
...
function showLoading() {
    document.querySelectorAll('canvas').forEach(canvas => {
        const container = canvas.parentElement;
        container.innerHTML = `
            <h3>${container.querySelector('h3') ? container.querySelector('h3').textContent : 'Carregando...'}</h3>
            <div class="loading">
